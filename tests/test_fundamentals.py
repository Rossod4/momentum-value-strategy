...
Uses small synthetic in-memory facts tables (matching the schema
get_company_facts() produces) rather than real SEC data, so these tests run
offline and stay fast and deterministic. Several tests reconstruct, in
miniature, real data quirks found during development against actual SEC
filings (e.g. Apple switching between a combined debt tag and split
current/noncurrent tags) - see the docstrings in fundamentals.py for the
full real-world context.
"""
...
def test_most_recent_instant_respects_filed_date():
    ...
    # (filed 2020-08-01) must not be visible yet, even though its `end`
    # date is in the past relative to nothing in particular.
...
def test_most_recent_instant_pools_alternate_tags_by_freshness():
    """Reconstructs the real Apple debt-tag-switch bug found during
    development: a company can stop updating one tag and start updating an
    alternate tag for the same concept. The freshest fact across BOTH
    candidate tags should win, not whichever tag happens to be tried
    first."""
    facts = make_facts(
        [
            {"tag": "LongTermDebt", "val": 40.0, "end": "2015-03-28", "filed": "2015-04-28"},
            {"tag": "LongTermDebtNoncurrent", "val": 101.0, "end": "2018-03-31", "filed": "2018-05-02"},
        ]
    )
...
def test_most_recent_instant_none_when_no_data_visible():
    facts = make_facts([{"tag": "StockholdersEquity", "val": 100.0, "end": "2020-03-31", "filed": "2020-05-01"}])
...
def test_total_debt_sums_split_current_and_noncurrent():
    facts = make_facts(
        [
            {"tag": "LongTermDebtNoncurrent", "val": 100.0, "end": "2020-06-30", "filed": "2020-08-01"},
...
def test_total_debt_zero_when_no_debt_tags_present():
    facts = make_facts([{"tag": "StockholdersEquity", "val": 100.0, "end": "2020-06-30", "filed": "2020-08-01"}])
...
def _quarterly_eps_facts(values: list[float], filed_lag_days: int = 30) -> list[dict]:
    ...
    for start, end, val in zip(starts, ends, values):
        ...
        rows.append({"tag": "EarningsPerShareDiluted", "val": val, "start": start, "end": end, "filed": filed})
    return rows
...
def test_ttm_duration_falls_back_to_annual_when_no_quarterly_data():
    facts = make_facts(
        [{"tag": "EarningsPerShareDiluted", "val": 4.5, "start": "2020-01-01", "end": "2020-12-31", "filed": "2021-02-15"}]
    )
...
def test_ttm_duration_restatement_uses_latest_filed_value_per_quarter():
    ...
    rows.append(
        {"tag": "EarningsPerShareDiluted", "val": 0.9, "start": "2020-01-01", "end": "2020-03-31", "filed": "2020-08-01"}
    )
...
def test_annual_growth_computed_from_two_most_recent_annual_facts():
    facts = make_facts(
        [
            {"tag": "EarningsPerShareDiluted", "val": 4.0, "start": "2018-01-01", "end": "2018-12-31", "filed": "2019-02-01"},
            {"tag": "EarningsPerShareDiluted", "val": 5.0, "start": "2019-01-01", "end": "2019-12-31", "filed": "2020-02-01"},
        ]
    )
...
def test_annual_growth_none_when_prior_year_zero():
    facts = make_facts(
        [
            {"tag": "EarningsPerShareDiluted", "val": 0.0, "start": "2018-01-01", "end": "2018-12-31", "filed": "2019-02-01"},
            {"tag": "EarningsPerShareDiluted", "val": 5.0, "start": "2019-01-01", "end": "2019-12-31", "filed": "2020-02-01"},
        ]
    )
    assert _annual_growth(facts, ["EarningsPerShareDiluted"], pd.Timestamp("2020-06-01")) is None
...
def test_diluted_eps_preferred_over_basic_when_both_present():
    ...
    facts = make_facts(
        [
            {"tag": "EarningsPerShareDiluted", "val": 4.0, "start": "2018-01-01", "end": "2018-12-31", "filed": "2019-02-01"},
            {"tag": "EarningsPerShareBasic", "val": 4.2, "start": "2018-01-01", "end": "2018-12-31", "filed": "2019-02-01"},
            {"tag": "EarningsPerShareDiluted", "val": 5.0, "start": "2019-01-01", "end": "2019-12-31", "filed": "2020-02-01"},
            {"tag": "EarningsPerShareBasic", "val": 5.3, "start": "2019-01-01", "end": "2019-12-31", "filed": "2020-02-01"},
        ]
    )
...
def test_basic_eps_used_as_fallback_when_diluted_entirely_absent():
    facts = make_facts(
        [
            {"tag": "EarningsPerShareBasic", "val": 4.0, "start": "2018-01-01", "end": "2018-12-31", "filed": "2019-02-01"},
            {"tag": "EarningsPerShareBasic", "val": 5.0, "start": "2019-01-01", "end": "2019-12-31", "filed": "2020-02-01"},
        ]
    )
    result = get_point_in_time_fundamentals(facts, "2020-06-01")
